...
import {Asset, Keypair, Memo, Operation, Transaction} from "@kinecosystem/kin-base";
import {KeyPair} from "./keyPair";
import {KinTransactionBuilder} from "./transactionBuilder";
import {NetworkError, ServerError, TransactionNotFoundError} from "../errors";
...
export class TxSender {
	...
	public async sendKin(address: Address, amount: string, fee: number, memoText?: string): Promise<Transaction> {
		const response: Server.AccountResponse = await this.server.loadAccount(this.keypair.publicAddress);
		return new KinTransactionBuilder(response, {fee: fee, memo: memoText ? Memo.text(memoText) : Memo.none()})
			.setTimeout(0)
